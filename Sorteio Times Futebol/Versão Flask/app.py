...
db_path = 'data/bancodedados.xlsx'
...
def adicionar_jogador(jogador_id):
    global jogadores_para_sorteio, contador_jogadores
    ...
    if jogador_selecionado:
        if jogador_id not in jogadores_adicionados:
            novo_jogador = {
                'id': jogador_selecionado['id'],
                'nome': jogador_selecionado['jogador'],
                'habilidade': float(jogador_selecionado['habilidade']),
                'posicao_primaria': jogador_selecionado['posicao_primaria'],
                'posicao_secundaria': jogador_selecionado['posicao_secundaria'],
                'adm': True if jogador_selecionado['diretor'].lower() == 'sim' else False
            }

            jogadores.append(novo_jogador)
            jogadores_adicionados.add(jogador_id)
            jogadores_para_sorteio.append(novo_jogador)
            jogadores_para_sorteio = jogadores_para_sorteio[-25:]
            contador_jogadores += 1

    return render_template('index.html', jogadores=jogadores_db, jogadores_para_sorteio=jogadores_para_sorteio, contador_jogadores=contador_jogadores)
...
def remover_jogador(jogador_id):
    ...
    jogador_selecionado = next(
        (jogador for jogador in jogadores if jogador['id'] == jogador_id), None)
