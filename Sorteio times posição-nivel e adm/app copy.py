...
def criar_times(jogadores, num_times):
    ...
    jogadores_adm_true = sorted(jogadores_adm_true, key=lambda x: x["habilidade"], reverse=True)
    jogadores_adm_false = sorted(jogadores_adm_false, key=lambda x: x["habilidade"], reverse=True)
    ...
    for jogador in jogadores_adm_true:
        ...
        menor_time = min(times, key=lambda t: (sum(j["habilidade"] for j in t), count_positions(t, jogador)))
...
def count_positions(time, jogador):
    count_primarias = sum(1 for j in time if j["posicao_primaria"] == jogador["posicao_primaria"])
    count_secundarias = sum(1 for j in time if j["posicao_secundaria"] == jogador["posicao_secundaria"])
...
def exibir_times(times):
    ...
    formatted_position_names = {'zagueiro': 'Zagueiros', 'meia': 'Meias', 'atacante': 'Atacantes', None: 'Nenhuma'}
    ...
    for i, time in enumerate(times, start=1):
        ...
        non_zero_primary_positions = {posicao: count_primary.get(posicao, 0) for posicao in order_of_positions if count_primary.get(posicao, 0) != 0}
        non_zero_secondary_positions = {posicao: count_secondary.get(posicao, 0) for posicao in order_of_positions if count_secondary.get(posicao, 0) != 0}

        # Usar o mapeamento para formatar os nomes das posições
        positions_primary_output = ', '.join(f'{formatted_position_names.get(posicao, posicao)}: {count}' for posicao, count in non_zero_primary_positions.items())
        positions_secondary_output = ', '.join(f'{formatted_position_names.get(posicao, posicao)}: {count}' for posicao, count in non_zero_secondary_positions.items())
        ...
        for jogador in time:
            ...
            posicao_secundaria = formatted_position_names.get(jogador['posicao_secundaria'], 'Nenhuma')
            ...
            print(f"  Jogador: {jogador['nome']} | Habilidade: {habilidade} | POS-1: {formatted_position_names[posicao_primaria]} | POS-2: {posicao_secundaria}")
